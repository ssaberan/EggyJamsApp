...
interface PointAndClickHotspotPropertiesPanelProps {
  hotspot: Hotspot;
...
export default function PointAndClickHotspotPropertiesPanel({
  hotspot,
  nodes,
  edges,
  variables,
  ...
  onUpdate,
  onDelete,
}: PointAndClickHotspotPropertiesPanelProps) {
  ...
  // For transition actions, look up whether an edge connects this hotspot to a target node
  ...
  }, [hasTransition, edges, currentNodeId, hotspot.id, nodes]);

  // Per-option connection status for showChoice actions
  ...
  const handleSetCondition = (condition: HotspotCondition | undefined) => {
    onUpdate({ condition });
  };

  const conditionVariable = hotspot.condition
    ? variables.find((v) => v.id === hotspot.condition!.variableId)
    : null;

  return (
    <div className="border-t border-gray-300 dark:border-gray-700 px-4 py-3 space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-[10px] font-semibold uppercase tracking-wider text-yellow-400">
          Properties
        </h4>
        <button
          onClick={onDelete}
          ...
          <div className="mt-2 flex flex-col gap-1.5 rounded-md border border-amber-500/20 bg-amber-500/5 px-2 py-2">
            ...
            <div className="flex gap-1.5">
              ...
              {conditionVariable?.type === 'boolean' ? (
                <select
                  value={String(hotspot.condition?.value ?? 'false')}
                  onChange={(e) => {
                    if (!hotspot.condition) return;
                    handleSetCondition({ ...hotspot.condition, value: e.target.value === 'true' });
                  }}
                  ...
                  onChange={(e) => {
                    ...
                    const val =
                      conditionVariable?.type === 'number'
                        ...
                        : e.target.value;
                    handleSetCondition({ ...hotspot.condition, value: val });
                  }}
                  placeholder="value"
                  className="flex-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-1.5 py-1 text-xs text-gray-700 dark:text-gray-300 placeholder-gray-600 outline-none focus:border-amber-500 transition-colors"
                />
              )}
    ...
    <div className="rounded-md border border-gray-300 dark:border-gray-700 bg-gray-800/50 px-2.5 py-2 space-y-2">
          ...
          <div className="flex gap-1.5">
            ...
            {actionVarDef?.type === 'boolean' ? (
              ...
              <input
                ...
                placeholder="value"
                className="flex-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-1.5 py-1 text-xs text-gray-700 dark:text-gray-300 placeholder-gray-600 outline-none focus:border-blue-500 transition-colors"
              />
            )}
          </div>
        </div>
      ...
      {action.type === 'showMessage' && (
        <textarea
          ...
          rows={2}
          className="w-full rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-2 py-1.5 text-xs text-gray-200 placeholder-gray-600 outline-none resize-y focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-colors"
        />
      )}
    </div>
  );
